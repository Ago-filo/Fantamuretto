...
const matchUnique=(rowPredicate,currentPredicate,reason)=>{
 const rows=activeRows.filter(row=>!matchedByRowId.has(playerId(row.Calciatore))&&rowPredicate(row));for(const row of rows){const candidates=current.filter(player=>!usedCurrentIds.has(player.id)&&player.role===row.Ruolo&&currentPredicate(player,row));if(candidates.length!==1)continue;const existing=candidates[0];matchedByRowId.set(playerId(row.Calciatore),existing);usedCurrentIds.add(existing.id);aliasMatches.push({from:existing.name,to:row.Calciatore,id:existing.id,reason})}
};
...
const clubCounters=new Map(),players=activeRows.map(row=>{
...
console.log(JSON.stringify({source:sourcePath,csvRows:csv.length,serieAPlayers:players.length,excludedWithoutClub:departedRows.length,matched,newPlayers:newPlayers.length,aliasMatches:aliasMatches.length,removed:removed.length,removedAsDeparted:removedAsDeparted.length,removedMissingFromCsv:removedMissingFromCsv.length,newExamples:newPlayers.slice(0,12).map(player=>player.name),aliasExamples:aliasMatches.slice(0,20),departedExamples:removedAsDeparted.slice(0,12).map(player=>player.name),missingExamples:removedMissingFromCsv.slice(0,20).map(player=>player.name)},null,2));
